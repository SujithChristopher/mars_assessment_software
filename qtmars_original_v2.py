...
class QtPluto(QObject):
    ...
    def _handle_stream(self, newdata: list[int]):
        ...
        # Run time - 5
        self.currstatedata.append(1e-3 * struct.unpack('L', bytes(newdata[6:10]))[0])
        ...
        self._currt = self.currstatedata[5]
        # print(f" {self.currstatedata[0]} Current / Previous time: {self._currt} / {self._prevt}")
        # if self._prevt is not None and isinstance(self._currt, float) and isinstance(self._prevt, float):
    ...
    def _handle_object_param(self, newdata: list[int]):
        """
